...
class ExperimentTemplate(PytorchExperiment):
    ...
    def setup_template(self):
        ...
        #self.train_datareader = torch.utils.data.Subset(self.train_datareader, range(100)).dataset
        #self.val_datareader = torch.utils.data.Subset(self.val_datareader, range(10)).dataset
        #self.test_datareader = torch.utils.data.Subset(self.test_datareader, range(10)).dataset
    ...
    def train(self, epoch, mort_pred_time=24):
        ...
        for batch_idx, batch in enumerate(train_batches):
            ...
            if self.config.dataset == 'MIMIC':
                ...
                if batch_idx%10==0:
                    print()
                    print(f'''train_batch_{batch_idx}: ''', end="")
                else:
                    print(batch_idx%10, end=" ")
                """
                flat: {flat.shape}
diagnoses: {diagnoses.shape}
padded: {padded.shape}"""
    ...
    def validate(self, epoch, mort_pred_time=24):
        ...
        if self.config.mode == 'train':
            ...
            for batch_idx, batch in enumerate(val_batches):
                ...
                if batch_idx%10==0:
                    print()
                    print(f'''val_batch_{batch_idx}: ''', end=" ")
                else:
                    print(batch_idx%10, end=" ")
                """
                flat: {flat.shape}
...
padded: {padded.shape}"""
    ...
    def test(self, mort_pred_time=24):
        ...
        for batch_idx, batch in enumerate(test_batches):
            ...
            if batch_idx%10==0:
                print()
                print(f'''test_batch_{batch_idx}: ''', end="")
            else:
                print(batch_idx%10, end=" ")
            """
            flat: {flat.shape}
