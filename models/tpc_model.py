...
class TempPointConv(nn.Module):
    ...
    def forward(self, X, diagnoses, flat, time_before_pred=5):
        ...
        if self.no_diag:
            ...
            # print(f'BEFORE ERROR_{i}')
            # print(flat.repeat_interleave(T - time_before_pred, dim=0).shape)
            # print(diagnoses_enc.repeat_interleave(T - time_before_pred, dim=0).shape)
            # print(next_X[:, :, time_before_pred:].permute(0, 2, 1).contiguous().view(B * (T - time_before_pred), -1).shape)
            # print()
            combined_features = cat((flat.repeat_interleave(T - time_before_pred, dim=0),  # (B * (T - time_before_pred)) * no_flat_features
