...
def gen_timeseries_file(eICU_path, test=False):
    ...
    for patient_chunk in gen_chunks:
        print(f'Started processing {i} patients')
        patient_chunk_tsl = [patient for patient in patient_chunk if patient in timeseries_lab.index.unique(level=0)]
        patient_chunk_tsr = [patient for patient in patient_chunk if patient in timeseries_resp.index.unique(level=0)]
        patient_chunk_tsn = [patient for patient in patient_chunk if patient in timeseries_nurse.index.unique(level=0)]
        patient_chunk_tsp = [patient for patient in patient_chunk if patient in timeseries_periodic.index.unique(level=0)]
        patient_chunk_tsap = [patient for patient in patient_chunk if patient in timeseries_aperiodic.index.unique(level=0)]

        merged = timeseries_lab.loc[patient_chunk_tsl].append(timeseries_resp.loc[patient_chunk_tsr], sort=False)
        merged = merged.append(timeseries_nurse.loc[patient_chunk_tsn], sort=False)
        merged = merged.append(timeseries_periodic.loc[patient_chunk_tsp], sort=False)
        merged = merged.append(timeseries_aperiodic.loc[patient_chunk_tsap], sort=True)
